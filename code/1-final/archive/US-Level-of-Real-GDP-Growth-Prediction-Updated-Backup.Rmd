...
download.file("https://raw.githubusercontent.com/jbalzani/US-Real-GDP-Prediction/master/data/0-raw/A191RL1Q225SBEA.csv", "A191RL1Q225SBEA.csv")
real_gdp_growth <- read_csv("A191RL1Q225SBEA.csv")
```

```{r import 10Y3M data, include=FALSE}
download.file("https://raw.githubusercontent.com/jbalzani/US-Real-GDP-Prediction/master/data/0-raw/T10Y3M.csv", "T10Y3M.csv")
T10Y3M <- read_csv("T10Y3M.csv")
```
...
for (i in 1:nrow(combined_data_eval)) {
  ...
  pred_reg_adl <- lm(real_gdp_growth ~ T10Y3M_lag2 + real_gdp_growth_lag + 
                    real_gdp_growth_lag2, data = train_window_adl)
  #predictions
...
for (i in 1:nrow(combined_data_eval)) {
  ...
  pred_reg_ar <- lm(real_gdp_growth ~ real_gdp_growth_lag + real_gdp_growth_lag2, 
                  data = train_window_ar)
  
  wf_predictions_ar[i] <- predict(pred_reg_ar, pred_window)
}
```
...
combined_data_eval %>% ggplot(aes(x = DATE)) +
  ...
  ggtitle("Figure 6: ADL(2, 1) Model Predicted vs. Actual Real GDP Growth Rates") #+
   #ggsave("change_this_name_later.png") #if this makes the file appear in Plot window, from      #there use Zoom button to get a high quality image, then right click, Copy Image, and        #save from MS Paint (best way so far)
```
...
combined_data_eval %>%
  ...
  #ggsave("test.png") #may show below and give open image option, screenshot directly from     there for highest quality image, copy image and save from paint if you need 2 images
  #of the same size
```
